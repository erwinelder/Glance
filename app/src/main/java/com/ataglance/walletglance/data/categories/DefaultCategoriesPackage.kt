...
data class DefaultCategoriesPackage(
    ...
    fun translateDefaultCategories(categoriesUiState: CategoriesLists): List<Category> {
        val translatedDefaultCategories = getDefaultCategories().concatenateLists()
        val currentCategoriesToTranslate = categoriesUiState.concatenateLists()
        ...
        return currentCategoriesToTranslate.map { currentCategory ->
            translatedDefaultCategories.findById(currentCategory.id)?.let{
                currentCategory.cloneWithNewName(it.name)
            } ?: currentCategory
        }
    }
